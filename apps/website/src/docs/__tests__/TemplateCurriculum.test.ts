...
const demonstratedContracts: Record<string, ReadonlyArray<string>> = {
  "render-your-first-template": [".value=${query}", "oninput=${readInput}", "RefSubject.make"],
  "authoring-typed-templates": [".value=${query}", "oninput=${readQuery}"],
  "renderable-normalization": ["Effect.succeed", "Stream.fromIterable", "Effect.Effect<string, E, R>"],
  ...
  "template-spreads-data": ["...${saveBindings}", ".data=${"],
  "dom-class-names": ["class=", "RefSubject.map"],
  "native-events-with-effect": ["EventHandler.make", "FormData", "preventDefault"],
  "template-references-and-element-access": ["Fx.callback", "RefSubject.set"],
  "template-namespaces-and-platform-markup": ["foreignObject", "xlink:href", "<math>"],
  "template-text-only-contexts": ["<textarea", "JSON.stringify", "<script"],
  ...
  "hydrating-typed-html": ["render(host)", "document.getElementById"],
  ...
  "render-scheduling": ["CurrentRenderPriority", "RenderPriority"],
  "render-event-substrate": ["DomRenderEvent", "Fx.sync"],
  "dom-render-event": ["DomRenderEvent", "Fx.periodic", "Effect.forkScoped"],
  "html-render-event": ["HtmlRenderEvent"],
  "wire-and-rendered-dom-output": ["persistent", "DomRenderEvent"],
