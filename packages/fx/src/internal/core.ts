...
class Make<A, E, R> extends FxBase<A, E, R> {
  ...
  run<R2>(sink: Sink.Sink<A, E, R2>): Effect.Effect<unknown, never, R | R2> {
    return Effect.contextWithEffect((ctx) =>
      this._run(Sink.provide(sink, ctx))
    );
  }
}
...
export const succeed = <A>(value: A): Fx<A> =>
  new Producer(SyncProducer.Success(value));

export const fromSync = <A>(f: () => A): Fx<A> =>
  new Producer(SyncProducer.FromSync(f));
...
export const fromScheduled = <R, E, I, R2, O>(
  ...
  new ProducerEffect(EffectProducer.FromScheduled(input, schedule));

export const schedule = <A, E, R, R2, O>(
  input: Effect.Effect<A, E, R>,
  schedule: Schedule.Schedule<O, unknown, R2>
): Fx<A, E, R | R2> =>
  new ProducerEffect(EffectProducer.Scheduled(input, schedule));
...
class ProducerSyncTransformer<A, E, R>
  extends FxBase<A, E, R>
  implements Fx<A, E, R>
{
  constructor(
...
export const map = <A, E, R, B>(fx: Fx<A, E, R>, f: (a: A) => B): Fx<B, E, R> =>
...
export const filterMapLoop = <A, E, R, B, C>(
  ...
  Transformer.make(fx, SyncLoopOp.FilterMapLoopOperator(seed, f));
...
export const loopEffect = <A, E, R, B, E2, R2, C>(
  ...
  Transformer.make(fx, EffectLoopOp.LoopEffectOperator(seed, f));
...
export const filterMapLoopEffect = <A, E, R, B, E2, R2, C>(
  ...
  Transformer.make(fx, EffectLoopOp.FilterMapLoopEffectOperator(seed, f));
...
export const drain = <A, E, R>(fx: Fx<A, E, R>): Effect.Effect<void, E, R> =>
  Observe.make(fx, constUnit);
...
class Observe<A, E, R, B, E2, R2> extends EffectBase<void, E | E2, R | R2> {
  ...
  toEffect(): Effect.Effect<void, E | E2, R | R2> {
    return Effect.asyncEffect((resume) => {
      ...
      const onFailure = (cause: Cause.Cause<E | E2>) =>
      ...
      return Op.matchOperator(fx.i1, {
          ...
          SyncOp.matchSyncOperator(op, {
            Map: (op): Effect.Effect<void, E | E2, R | R2> =>
              SyncProducer.effectOnce(() =>
                SyncProducer.runEffect(fx.i0, (a) => f(op.f(a)))
              ),
            Filter: (op) =>
              SyncProducer.effectOnce(() =>
        ...
        EffectLoopOperator: (op) =>
          EffectLoopOp.matchEffectLoopOperator(op, {
            LoopEffect: (op) =>
              SyncProducer.runReduceEffect(fx.i0, op.seed, (acc, a) =>
                Effect.flatMap(op.f(acc, a), ([c, b]) => Effect.as(f(c), b))
              ),
      ...
      return Op.matchOperator(fx.i1, {
        SyncOperator: (op): Effect.Effect<void, E | E2, R | R2> =>
          SyncOp.matchSyncOperator(op, {
            Map: (op): Effect.Effect<void, E | E2, R | R2> =>
              SyncProducer.effectOnce(() =>
                EffectProducer.runEffect(fx.i0, (a) => f(op.f(a)))
              ),
            Filter: (op) =>
              SyncProducer.effectOnce(() =>
...
class Reduce<A, E, R, B> extends EffectBase<B, E, R> {
  ...
  static make<A, E, R, B>(fx: Fx<A, E, R>, seed: B, f: (acc: B, a: A) => B) {
    ...
    if (isEmpty(fx)) return Effect.succeed(seed);
      ...
      return Op.matchOperator(fx.i1, {
        SyncOperator: (op) =>
          SyncProducer.syncOnce(() => SyncOp.runSyncReduce(fx.i0, op, seed, f)),
      ...
      return EffectProducer.runReduceEffect(fx.i0, seed, (b, a) =>
        Effect.succeed(f(b, a))
      );
    } else if (isFailCause(fx)) {
      return Effect.failCause(fx.i0);
    } else {
...
export const take = <A, E, R>(fx: Fx<A, E, R>, n: number): Fx<A, E, R> =>
  slice(fx, 0, n);

export const drop = <A, E, R>(fx: Fx<A, E, R>, n: number): Fx<A, E, R> =>
  slice(fx, n, Infinity);
  ...
  switch (typeof u) {
    case "object": {
      if (Predicate.isNullable(u)) {
        ...
        return Data.tuple(
          Array.from(u, ([k, v]) =>
            Data.tuple([toDeepEquals(k), toDeepEquals(v)])
          )
        );
      } else if (u instanceof URLSearchParams) {
        return Data.tuple(
          Array.from(u.keys()).map((key) =>
            Data.tuple([key, toDeepEquals(u.getAll(key))])
          )
        );
...
type YieldWrapError<T> =
  T extends Utils.YieldWrap<Effect.Effect<infer _, infer E, any>> ? E : never;
type YieldWrapContext<T> =
  T extends Utils.YieldWrap<Effect.Effect<any, any, infer R>> ? R : never;
...
class OrElseCause<A, E, R, R2, E2, B> extends FxBase<A | B, E2, R | R2> {
  ...
  run<R3>(
    ...
    return Effect.catchAllCause(observe(this.i0, sink.onSuccess), (cause) =>
      this.i1(cause).run(sink)
    );
  }
    ...
    return withScopedFork(
      (fork) =>
        Sink.withEarlyExit(sink, (s) => {
          ...
          const onFailure = (cause: Cause.Cause<E | E2 | E3>) =>
            s.onFailure(cause);
          ...
          return Effect.flatMap(
            ...
            () =>
              Effect.zipRight(
                ...
                adjustTime(1),
                this.i0.run(
                  Sink.make(onFailure, (a) =>
                    taking ? s.onSuccess(a) : Effect.void
                  )
                )
              )
          );
...
export function annotateLogs<A, E, R>(
  ...
  return middleware(fx, (effect) =>
    Effect.annotateLogs(effect, key as string, value as unknown)
  );
}
...
export function annotateSpans<A, E, R>(
  ...
  return middleware(fx, (effect) =>
    Effect.annotateSpans(effect, key as string, value as unknown)
  );
}

export const interruptible = <A, E, R>(fx: Fx<A, E, R>): Fx<A, E, R> =>
  middleware(fx, Effect.interruptible);

export const uninterruptible = <A, E, R>(fx: Fx<A, E, R>): Fx<A, E, R> =>
  middleware(fx, Effect.uninterruptible);
...
export function withConcurrency<A, E, R>(
  ...
  return middleware(fx, (effect) =>
    Effect.withConcurrency(effect, concurrency)
  );
}
...
export function withConfigProvider<A, E, R>(
  ...
  return middleware(fx, (effect) =>
    Effect.withConfigProvider(effect, configProvider)
  );
}
...
export function withMaxOpsBeforeYield<A, E, R>(
  ...
  return middleware(fx, (effect) =>
    Effect.withMaxOpsBeforeYield(effect, maxOps)
  );
}
...
export function withRequestBatching<A, E, R>(
  ...
  return middleware(fx, (effect) =>
    Effect.withRequestBatching(effect, requestBatching)
  );
}
...
export function withRequestCaching<A, E, R>(
  ...
  return middleware(fx, (effect) =>
    Effect.withRequestCaching(effect, requestCaching)
  );
}
  ...
  run<R4>(
    ...
    return Effect.catchAllCause(
      Effect.acquireUseRelease(
        ...
        (a, exit) =>
          Effect.catchAllCause(this.release(a, exit), (cause) =>
            sink.onFailure(cause)
          )
      ),
  ...
  return acquireUseRelease(
    ...
    (span, exit) =>
      Effect.flatMap(Clock.currentTimeNanos, (time) =>
        Effect.sync(() => span.end(time, exit))
      )
  );
}
...
export function filterMapLoopErrorEffect<A, E, R, B, E2, R2, C>(
  ...
  return filterMapLoopCauseEffect(fx, seed, (b, cause) =>
    Either.match(Cause.failureOrCause(cause), {
      onLeft: (e) =>
        Effect.map(f(b, e), ([c, b2]) => [Option.map(c, Cause.fail), b2]),
...
> {
  const entries: ReadonlyArray<readonly [keyof FX, FX[keyof FX]]> =
    Object.entries(fx) as any;
...
function emitAndSleep<A>(value: A, delay: Duration.DurationInput) {
  return make<A>((sink) =>
    Effect.zipRight(sink.onSuccess(value), Effect.sleep(delay))
  );
}
...
export function findFirst<A, E, R>(
  ...
  return Effect.asyncEffect((cb) =>
    observe(fx, (a) =>
      predicate(a) ? Effect.sync(() => cb(Effect.succeed(a))) : Effect.void
    )
  );
}
      ...
      const fiber1 = yield* Effect.fork(
        this.i0.run(
          Sink.make(sink.onFailure, (a) =>
            Effect.flatMap(sink.onSuccess(a), () => Fiber.interrupt(fiber2))
          )
        )
      );
    ...
    return Effect.gen(this, function* (_) {
      ...
      for (const fx of this.i0) {
        const fiber: Fiber.RuntimeFiber<unknown> = yield* Effect.fork(
          fx.run(
            Sink.make(sink.onFailure, (a) =>
              Effect.flatMap(Deferred.succeed(winner, fiber), () =>
                sink.onSuccess(a)
              )
            )
          )
...
export function when<B, E, R, C>(
  ...
  return map(skipRepeatsWith(bool, boolean), (b) =>
    b ? options.onTrue : options.onFalse
  );
}
