  ...
  return debounce(
    (forkDebounce, parentScope) =>
      withExhaustLatestFork(
        (fork) => {
          let previousKeys: ReadonlyArray<B> = []
          const entries = new Map<B, KeyedEntry<A, C>>()
          const indices = new Map<number, B>()

          function getReadyIndices(): ReadonlyArray<C> {
          ...
          function handleOutput() {
            return forkDebounce(Effect.suspend(() => sink.onSuccess(getReadyIndices())))
          }

          function addValue(values: ReadonlyArray<A>, patch: Add<B>) {
          ...
          function updateValue(values: ReadonlyArray<A>, patch: Update<B> | Moved<B>) {
          ...
          function removeValue(patch: Remove<B>) {
          ...
          function diffAndPatch(values: ReadonlyArray<A>) {
            ...
            return Effect.flatMap(
              Effect.forEach(diff(previous, keys), (patch) => {
                if (patch._tag === "Remove") return removeValue(patch)
                else if (patch._tag === "Add") {
                  added = true
                  return addValue(values, patch)
                } else return updateValue(values, patch)
              }, DISCARD),
              () => added === false ? handleOutput() : Effect.unit
            )
          }
