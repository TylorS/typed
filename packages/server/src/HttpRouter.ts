...
import {
  ...
  HttpRouter as PlatformHttpRouter,
} from "@effect/platform";
...
export namespace HttpRouter {
  ...
  export interface TagClass<Self, Name extends string, E, R>
    extends Context.TagClass<Self, Name, Service<E, R>> {
...
> => {
  ...
  for (const routeHandler of router.routes) {
    platformRouter =
      HttpRouteHandler.toPlatformRoute(routeHandler)(platformRouter);
  }
...
export const catchAllCause: {
  ...
  <E, R, E2, R2>(
    ...
    new RouterImpl(
      Chunk.map(router.routes, (handler) =>
        HttpRouteHandler.catchAllCause(handler, onCause)
      ),
      Chunk.map(
        router.mounts,
        (mount) =>
...
export const catchAll: {
  ...
  <E, R, E2, R2>(
    ...
    new RouterImpl(
      Chunk.map(router.routes, (handler) =>
        HttpRouteHandler.catchAll(handler, onCause)
      ),
      Chunk.map(
        router.mounts,
        (mount) =>
...
export const catchTag: {
  ...
  >(
    ...
    new RouterImpl(
      Chunk.map(router.routes, (handler) =>
        HttpRouteHandler.catchTag(handler, tag, onError)
      ),
      Chunk.map(
        router.mounts,
        (mount) =>
...
export const catchTags: {
  ...
  >(
    ...
    new RouterImpl(
      Chunk.map(router.routes, (handler) =>
        HttpRouteHandler.catchTags(handler, cases)
      ),
      Chunk.map(
        router.mounts,
        (mount) =>
