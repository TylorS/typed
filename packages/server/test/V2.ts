...
import { HttpApi, HttpApiBuilder, HttpApiEndpoint, HttpApiGroup } from "../src/V2"
import { HttpApiBuilderRouter } from "../src/V2/HttpApiBuilder"
...
describe("V2", () => {
  it.effect("should work", () =>
    Effect.gen(function*() {
      ...
      const barHandler = barEndpoint.pipe(HttpApiEndpoint.handle(({ path }) => Effect.succeed(`Got bar ${path.barId}`)))
      ...
      const Api = HttpApi.empty.pipe(
        HttpApi.addGroup(FooBar)
      )

      const FooBarLive = HttpApiBuilder.group(Api, `FooBar`, flow(fooHandler, barHandler))
      const ApiLive = HttpApiBuilder.api(Api).pipe(
        Layer.provide(FooBarLive)
      )

      const port = yield* Effect.promise(() => getPort()) // Get an available port

      const ServerLive = ApiLive.pipe(
        Layer.provide(NodeHttpServer.layer(createServer, { port: port as number })), // Use the available port
      ...
      const fooResponse = yield* Effect.promise(() =>
        handler(new Request(`http://localhost:${port}/foo/123`)).then((r) => r.json()) // Use the dynamic port
      )

      expect(fooResponse).toEqual("Got foo 123")

      const barResponse = yield* Effect.promise(() =>
        handler(new Request(`http://localhost:${port}/bar/123`)).then((r) => r.json()) // Use the dynamic port
      )

      expect(barResponse).toEqual("Got bar 123")

      const fooResponse2 = yield* Effect.promise(() => handler(new Request(`http://localhost:${port}/foo/asdf`))) // Use the dynamic port
