...
describe("Parser", () => {
  ...
  it("returns an independent AST for each public parse call", () => {
    ...
    assert.notStrictEqual(Parser.parse(template), Parser.parse(template));
  });
  ...
  it.each([
    ["event", h`<div @click="before-${"handler"}"></div>`],
    ["boolean", h`<div ?hidden="before-${true}"></div>`],
    ["data", h`<div .data="before-${{ value: "x" }}"></div>`],
    ["property", h`<div .value="before-${"x"}"></div>`],
    ["ref", h`<div ref="before-${() => {}}"></div>`],
    ["spread", h`<div ...${{ title: "x" }}suffix></div>`],
  ])("rejects sparse %s directives", (_name, template) => {
    expect(() => Parser.parse(template)).toThrowError(/must contain exactly one interpolation/);
  });

  it.each([
    ["event", h`<div @click="handler"></div>`],
    ["boolean", h`<div ?hidden="yes"></div>`],
    ["data", h`<div .data="value"></div>`],
    ["property", h`<div .value="value"></div>`],
    ["ref", h`<div ref="value"></div>`],
    ["spread", h`<div ...props></div>`],
  ])("rejects static %s directives", (_name, template) => {
    expect(() => Parser.parse(template)).toThrowError(/must contain exactly one interpolation/);
  });
