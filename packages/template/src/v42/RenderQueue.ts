...
export interface RenderQueue {
  readonly add: (part: unknown, task: RenderTask, priority: number) => Effect.Effect<void, never, Scope.Scope>
}
...
class PriorityQueue {
  priorities: Map<number, Map<unknown, RenderTask>> = new Map()

  add(part: unknown, task: RenderTask, priority: number) {
...
abstract class BaseImpl implements RenderQueue {
  ...
  protected runTasks = (iterator: Iterator<Iterable<RenderTask>>) => {
  ...
  protected tryRunTask = (task: RenderTask) => {
    try {
      task.run()
    } catch (error) {
      task.onError(error)
...
class SyncImpl implements RenderQueue {
  ...
  add(_: unknown, task: RenderTask) {
    return Effect.sync(() => this.runTask(task))
  }

  private runTask(task: RenderTask) {
    try {
      task.run()
    } catch (error) {
      task.onError(error)
