...
Each render creates a fresh Vue app and SSR context. Native `render` hydrates matching server hosts with `createSSRApp`; it preserves server node identity, including neighboring islands. Use the same components, props, explicit ID overrides, plugins, and service state on the server and client. Provide native `StaticHtmlRenderTemplate` for build-time output without Typed hydration markers.
...
`Typed` owns an automatic host with a Vue `useId` identity (or the supplied `id` prop) and hydrates its existing nodes after Vue mounts. Its opaque slot body needs a complete HTML snapshot during `onServerPrefetch`; the surrounding Vue tree can stream up to that slot while it is pending. The component provides the ordinary Typed DOM or HTML renderer through Vue's lifecycle. Updating the `value` prop or injected runtime aborts the previous render and waits for its finalizers before starting the next. Unmount stops rendering without disposing the shared runtime. Failures go to Vue's error boundary, or to the optional `onError` callback with their complete Effect cause.

`onError` preserves the view's failure type. A component from `createTypedComponent(runtime)` also includes the borrowed runtime's failure type. Vue TSX infers these from `value`:

```tsx
const value = Effect.fail({ _tag: "Unavailable" as const });
const result = <Typed value={value} onError={(cause) => console.error(cause)} />;
```

With Vue's `h`, specialize the generic component when the callback needs the concrete error type: `h(Typed<typeof value>, { value, onError: (cause) => console.error(cause) })`. The inherited runtime's service and failure types are erased by Vue injection; use `createTypedComponent(runtime)` for an explicit runtime contract. Reactive composables such as `useEffect` also accept `onError`, retaining both source and explicit runtime failures.
