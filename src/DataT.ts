...
export function loading<F extends URIS>(P: Pointed1<F>): Kind<F, D.Loading>
export function loading<F>(P: Pointed<F>): HKT<F, D.Loading>
...
export function fromProgress<F extends URIS3, R = Initial<F, 'R'>, E = Initial<F, 'E'>>(
  P: Pointed3<F>,
): (progress: Progress) => Kind3<F, R, E, D.Loading>
export function fromProgress<F extends URIS2, E = Initial<F, 'E'>>(
  P: Pointed2<F>,
): (progress: Progress) => Kind2<F, E, D.Loading>
export function fromProgress<F extends URIS>(
  P: Pointed1<F>,
): (progress: Progress) => Kind<F, D.Loading>
export function fromProgress<F>(P: Pointed<F>): (progress: Progress) => HKT<F, D.Loading>
export function fromProgress<F>(P: Pointed<F>) {
...
export function getOrElseW<F extends URIS3>(
  F: Functor.Functor3<F>,
): <A, B>(
  onNoData: () => A,
  onLoading: (progress: O.Option<Progress>) => B,
) => <R, E, C>(ma: Kind3<F, R, E, D.Data<C>>) => Kind3<F, R, E, A | B | C>
export function getOrElseW<F extends URIS2>(
  F: Functor.Functor2<F>,
): <A, B>(
  onNoData: () => A,
  onLoading: (progress: O.Option<Progress>) => B,
) => <E, C>(ma: Kind2<F, E, D.Data<C>>) => Kind2<F, E, A | B | C>
export function getOrElseW<F extends URIS>(
  F: Functor.Functor1<F>,
): <A, B>(
  onNoData: () => A,
  onLoading: (progress: O.Option<Progress>) => B,
