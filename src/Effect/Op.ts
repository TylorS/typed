 ...
 */
export interface Op<Key = any, Args extends ReadonlyArray<any> = any, R = any>
  extends Newtype<OpUri<Args, R>, Key> {}

export interface OpUri<Args extends ReadonlyArray<any> = any, R = any> {
...
export function provideOp<O extends Op, E>(Op: O, implementation: GetOp<E, O>) {
  return <F, A>(eff: Effect<F & OpEnv<O>, A>): Effect<E & F, A> => {
    const effect = doEffect(function* () {
      ...
      opEnvIso.unwrap(opEnv)[OP].set(Op, implementation)
...
export function useOp<O extends Op>(Op: O) {
  return (...args: OpArgs<O>): Effect<OpEnv<O>, OpReturn<O>> => {
    return doEffect(function* () {
      ...
      const op = map.get(Op)! as GetOp<unknown, O>
      const value = yield* op(...args)
...
export function performOp<K, R>(Op: Op<K, readonly [], R>): Effect<OpEnv<typeof Op>, R>
