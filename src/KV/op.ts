...
export function op<F extends URIS4>(
  ...
  key: K,
) => <S, R, E, A>(f: (op: Op) => Kind4<F, S, R, E, A>) => Kind4<F, S, WidenI<GetKV4<F> | R>, E, A>

export function op<F extends URIS3>(
  M: MonadReader3<F>,
): <Op>() => <K extends PropertyKey>(
  key: K,
) => <R, E, A>(f: (op: Op) => Kind3<F, R, E, A>) => Kind3<F, WidenI<GetKV3<F> | R>, E, A>

export function op<F extends URIS3, E>(
  M: MonadReader3C<F, E>,
): <Op>() => <K extends PropertyKey>(
  key: K,
) => <R, A>(f: (op: Op) => Kind3<F, R, E, A>) => Kind3<F, WidenI<GetKV3<F> | R>, E, A>

export function op<F extends URIS2>(
  M: MonadReader2<F>,
): <Op>() => <K extends PropertyKey>(
  key: K,
) => <E, A>(f: (op: Op) => Kind2<F, E, A>) => Kind2<F, WidenI<GetKV2<F> | E>, A>

export function op<F>(
  M: MonadReader<F>,
): <Op>() => <K extends PropertyKey>(
  key: K,
) => <E, A>(f: (op: Op) => HKT2<F, E, A>) => HKT2<F, WidenI<GetKV<F> | E>, A>
