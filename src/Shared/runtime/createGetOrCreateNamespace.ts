...
): <K extends PropertyKey>(namespace: Namespace<K>) => Kind2<F, RuntimeEnv<F>, Map<any, any>>
...
): <K extends PropertyKey>(namespace: Namespace<K>) => Kind3<F, RuntimeEnv<F>, E, Map<any, any>>

export function createGetOrCreateNamespace<F extends URIS4>(
  M: MonadAsk4<F> & FromIO4<F>,
): <K extends PropertyKey, S, E = never>(
  namespace: Namespace<K>,
) => Kind4<F, S, RuntimeEnv<F>, E, Map<any, any>>

export function createGetOrCreateNamespace<F>(
  M: MonadAsk<F> & FromIO<F>,
): <K extends PropertyKey>(namespace: Namespace<K>) => HKT<F, Map<any, any>>

export function createGetOrCreateNamespace<F>(M: MonadAsk<F> & FromIO<F>) {
