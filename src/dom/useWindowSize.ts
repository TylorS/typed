...
export const useWindowSize = DoF(function* (_) {
  ...
  const listener = yield* _(
    H.useOp(() => pipe(window, getWindowSize, Size.set, withFiberRefs(fiber))),
  )
  const f = yield* _(H.useFn(flow(listener, exec)))

  yield* _(useEventListener(window, 'resize', f, options))
