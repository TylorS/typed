...
export function createUseContext<F>(M: MonadReader<F> & FromIO<F> & UseSome<F> & ChainRec<F>) {
  ...
  return <K, E, A>(shared: KV<F, K, E, A>) =>
    withProvider(shared, (provider) => pipe(shared, getKV, using(provider)))
}
