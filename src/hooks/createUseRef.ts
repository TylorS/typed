...
import { createGetKV, Shared } from '@typed/fp/Shared'
...
export function createUseRef<F extends URIS2>(
  ...
  <A>(): Kind2<F, Shared<F>, Ref<A | undefined>>
  <A>(value: A): Kind2<F, Shared<F>, Ref<A>>
}

export function createUseRef<F extends URIS3>(
  M: MonadReader3<F> & FromIO3<F>,
): {
  <A, E = never>(): Kind3<F, Shared<F>, E, Ref<A | undefined>>
  <A, E = never>(value: A): Kind3<F, Shared<F>, E, Ref<A>>
}

export function createUseRef<F extends URIS3, E>(
  M: MonadReader3C<F, E> & FromIO3C<F, E>,
): {
  <A>(): Kind3<F, Shared<F>, E, Ref<A | undefined>>
  <A>(value: A): Kind3<F, Shared<F>, E, Ref<A>>
}

export function createUseRef<F extends URIS4>(
  M: MonadReader4<F> & FromIO4<F>,
): {
  <A, S = unknown, E = never>(): Kind4<F, S, Shared<F>, E, Ref<A | undefined>>
  <A, S = unknown, E = never>(value: A): Kind4<F, S, Shared<F>, E, Ref<A>>
}

export function createUseRef<F>(
  M: MonadReader<F> & FromIO<F>,
): {
  <A>(): HKT2<F, Shared<F>, Ref<A | undefined>>
  <A>(value: A): HKT2<F, Shared<F>, Ref<A>>
}
